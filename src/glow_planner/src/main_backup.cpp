...
class ImageConverter
{
		...
		: it_(nh_)
	{
		...
		//http://ninghang.blogspot.in/2011/11/tutorial-1-use-ros-to-get-image-with.html
		char temp[25];
		...
		//TODO:find a more elegant solution to scale this.//use template
		...
		bot_pose_sub[5]=nh_.subscribe(bot_pose_topic[5],1,&ImageConverter::bot5PoseCb,this);
		bot_pose_sub[6]=nh_.subscribe(bot_pose_topic[6],1,&ImageConverter::bot6PoseCb,this);
...
//			}
		...
		target_intensity=100; // doubt is this the boundary intensity??the boundary along which bots have to move eventually
	...
	void bot5PoseCb(const geometry_msgs::Pose2DConstPtr& msg)
	{
		bot_pose[5].x=msg->x;
		bot_pose[5].y=msg->y;
		bot_pose[5].theta=msg->theta;
	}
	void bot6PoseCb(const geometry_msgs::Pose2DConstPtr& msg)
	{
		bot_pose[6].x=msg->x;
		bot_pose[6].y=msg->y;
		bot_pose[6].theta=msg->theta;
	}	
	...
	void imageCb(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		*/
		...
		catch (cv_bridge::Exception& e)
		{
			...
			if(!plot_start)
			{
				path=Mat::zeros(image.size(), CV_8UC3);//8bit 3 channel unsigned char cv_8uc3
		...
		if(state==ROTATE&&frame_count>2)
		{
			...
			for(int i=0;i<NO_BOTS;i++)	 
			{
				...
				if(abs(error_angle_degrees)>5) //doubt should check for > than 180 also as atan2 has domain of -pi to pi
					//so suppose error_angle=goal_heading[i] - bot_pose[i].theta; goal_heading[i]= - 179,  bot_pose = +179 then this angle will come -358 = + 2
					//was this the reason why u might be getting segmentation error
				{
					//Correct heading;
					...
					if(error_angle_degrees<0.0f)
						nxt_command[i].angular.z=( error_angle_degrees/180.0f)-0.5f;//doubt pi multiply
					else
